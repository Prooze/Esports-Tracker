...
router.post('/', checkPermission('manage_upcoming'), (req, res, next) => {
  try {
...
router.put('/:id', checkPermission('manage_upcoming'), (req, res, next) => {
  try {
    const err = validateBody(req.body);
    if (err) return sendError(res, 400, err);
...
router.delete('/:id', checkPermission('manage_upcoming'), (req, res, next) => {
  try {
...
router.post('/:id/dismiss', checkPermission('manage_upcoming'), (req, res, next) => {
  try {
